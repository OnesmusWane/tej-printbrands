@php
    ...
    $blankRowsNeeded = max(0, 8 - $items->count());
    $hasPayment = $inv->paid_amount > 0;
@endphp
<!DOCTYPE html>
  ...
  <div style="position:relative;width:794px;height:210px;">
    ...
    <div style="position:absolute;left:26px;top:24px;">
      <table style="width:auto;">
        <tr>
          <td style="width:82px;">
            <div style="width:82px;height:82px;border-radius:50%;background:#fff;overflow:hidden;text-align:center;line-height:82px;">
              @if ($logoUrl)
    ...
    <div style="position:absolute;right:30px;bottom:10px;width:300px;">
      <div style="font-size:30px;font-weight:700;letter-spacing:2px;text-align:right;margin-bottom:8px;">INVOICE</div>
      <table style="font-size:11px;">
        <tr><td style="border:1px solid #999;padding:4px 10px;width:90px;">Date:</td><td style="border:1px solid #999;padding:4px 10px;">{{ $inv->created_at->format('d M Y') }}</td></tr>
        <tr><td style="border:1px solid #999;padding:4px 10px;">Invoice #:</td><td style="border:1px solid #999;padding:4px 10px;">{{ $inv->invoice_number }}</td></tr>
        <tr><td style="border:1px solid #999;padding:4px 10px;">Due:</td><td style="border:1px solid #999;padding:4px 10px;">{{ $inv->due_date?->format('d M Y') ?? '—' }}</td></tr>
        <tr><td style="border:1px solid #999;padding:4px 10px;">Bill To:</td><td style="border:1px solid #999;padding:4px 10px;">{{ $inv->client }}</td></tr>
      </table>
    </div>
  </div>
  ...
  <div style="padding:26px 26px 8px;">
    <table>
      ...
      <tbody>
        ...
        @empty
          <tr>
            <td colspan="4" style="padding:14px;border-bottom:1px solid #00BCD4;color:#6B7280;font-style:italic;text-align:center;">{{ $inv->service ?: 'Professional services' }}</td>
          </tr>
        @endforelse
    ...
    <!-- Right-aligned breakdown + amount due / balance due bar, via a spacer column instead of flexbox -->
    <table style="margin-top:8px;">
      <tr>
        <td style="width:52%;">&nbsp;</td>
        <td style="width:48%;">
          @if ($inv->vat_included !== false)
            <table>
              <tr><td style="padding:4px 16px;font-size:12px;color:#6B7280;">Subtotal</td><td style="padding:4px 16px;font-size:12px;text-align:right;">Ksh {{ number_format($inv->subtotal) }}</td></tr>
              <tr><td style="padding:4px 16px;font-size:12px;color:#6B7280;{{ $hasPayment ? '' : 'border-bottom:1px solid #e5e7eb;' }}">VAT (16%)</td><td style="padding:4px 16px;font-size:12px;text-align:right;{{ $hasPayment ? '' : 'border-bottom:1px solid #e5e7eb;' }}">Ksh {{ number_format($inv->tax) }}</td></tr>
            </table>
          @endif
          @if ($hasPayment)
            <table>
              <tr><td style="padding:4px 16px;font-size:12px;color:#6B7280;">Amount Paid</td><td style="padding:4px 16px;font-size:12px;text-align:right;color:#16a34a;">- Ksh {{ number_format($inv->paid_amount) }}</td></tr>
            </table>
          @endif
          <table style="background:#111;margin-top:6px;">
            <tr>
              <td style="padding:10px 16px;color:#fff;font-weight:700;font-size:14px;">{{ $hasPayment ? 'Balance Due' : 'Amount Due' }}{{ $inv->vat_included === false ? ' (VAT Exempt)' : '' }}</td>
              <td style="padding:10px 16px;color:#00BCD4;font-weight:700;font-size:14px;text-align:right;">Ksh {{ number_format($inv->balance_due) }}</td>
            </tr>
          </table>
        </td>
      </tr>
    ...
    @if ($inv->terms)
      <div style="margin-top:14px;font-size:11px;color:#6B7280;padding-top:10px;border-top:1px solid #e5e7eb;"><strong>Terms &amp; Conditions:</strong> {{ $inv->terms }}</div>
    @endif

    @if ($inv->payments && $inv->payments->isNotEmpty())
          ...
          <tbody>
            @foreach ($inv->payments as $p)
              <tr>
                ...
                <td style="padding:5px 10px;font-size:11px;border-bottom:1px solid #f3f4f6;">{{ ucfirst($p->method) }}</td>
                <td style="padding:5px 10px;font-size:11px;border-bottom:1px solid #f3f4f6;">{{ $p->paid_at?->format('d M Y') }}</td>
                <td style="padding:5px 10px;font-size:11px;text-align:right;border-bottom:1px solid #f3f4f6;">Ksh {{ number_format($p->amount) }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
