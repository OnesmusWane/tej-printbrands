        ...
        <div class="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[260px_1fr] lg:px-8">
            ...
            <div class="grid gap-8 lg:grid-cols-[1fr_320px]">
                <div class="rounded-2xl bg-white p-6 shadow-sm">
                    ...
                    <div class="space-y-4">@foreach ($order->items as $item)<div class="rounded-xl border border-slate-100 p-4"><div class="flex justify-between gap-4"><span class="font-bold text-slate-900">{{ $item['product']['name'] }}</span><span>Ksh {{ number_format($item['line_total']) }}</span></div><p class="mt-1 text-sm text-slate-500">{{ $item['quantity'] }} x {{ $item['finish'] }}</p></div>@endforeach</div>
                </div>
                <aside class="space-y-5">
                    <div class="rounded-2xl bg-white p-6 shadow-sm"><h3 class="mb-4 text-xl font-extrabold text-slate-900">Summary</h3><p class="flex justify-between text-sm"><span>Subtotal</span><strong>Ksh {{ number_format($order->subtotal) }}</strong></p><p class="mt-2 flex justify-between text-sm"><span>Handling</span><strong>Ksh {{ number_format($order->service_fee) }}</strong></p><p class="mt-4 border-t pt-4 text-2xl font-extrabold text-primary">Ksh {{ number_format($order->total) }}</p></div>
